...
enum JITHelperClient {
    ...
    static func send(
        ...
        JITHelperLauncher.launch(
            withPayload: request.userInfo,
            timeout: timeout
        ) { helperPid, replyItems, error in
            if let error {
                Log.jit.note(
                    "host: \(operation) failed after \(elapsed()) "
            ...
            guard let reply = JITHelper.Reply(replyItems: replyItems) else {
                Log.jit.note(
                    "host: \(operation) replied with something unreadable "
                        + "after \(elapsed())")
                completion(nil)
                return
            }
